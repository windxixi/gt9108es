...
//#    define SAMSUNG_KIES_PRODUCT_ID	0x685C	/* mtp only  for T1_CHN : MTP + ACM -> MTP only */
#  endif
...
//#    define SAMSUNG_UMS_PRODUCT_ID	0x685e  /* UMS Only(x) UMS + ACM for T1_CHN : UMS only -> UMS + ACM  - for KIES*/
...
//#    define ANDROID_KIES_CONFIG_STRING	 "MTP only (for T1_CHN  : MTP + ACM -> MTP only)"  // for T1_CHN : MTP + ACM -> MTP only
#  endif
...
//#  define       ANDROID_UMS_CONFIG_STRING	 "UMS + ACM for T1_CHN : UMS only -> UMS + ACM (Not debugging mode)"
