...
extern void max14577_usbpath_change(int usb_path);
...
)
{
	...
	printk("\n[usb_switch_store][buf=%s]\n", buf);

#ifdef PARAM_SWITCH_SEL
	if (sec_get_param_value)
	{
		sec_get_param_value(__SWITCH_SEL, &switch_sel);
		printk("[%s] get switch_sel=%d\n", __func__,  switch_sel);
	}
	else	{
		printk("[%s] CAN'T GET SWITCH_SEL VALUE\n", __func__);
	}
#endif

	if(strstr(buf, "PDA") || strstr(buf, "pda") || strstr(buf, "1"))
	{
	...
	else if(strstr(buf, "MODEM") || strstr(buf, "modem") || strstr(buf, "0"))
	{
	...
	if(path_save)
	{
		...
		if (sec_set_param_value)
		{
			...
			printk("[%s] set switch_sel=%d\n", __func__, switch_sel);
		}
		else	{
			printk("[%s] CAN'T SET SWITCH_SEL VALUE\n", __func__);
		}
#endif
	}
...
)
{
	...
	printk("[uart_switch_store]buf=%s]\n@\n", buf);
...
static void sio_switch_init_worker(struct work_struct *ignored)
{
	...
	if (sec_get_param_value) {
		...
		cancel_delayed_work(&sio_switch_init_work);
		 ...
		 /* Reset SWITCH_SEL value to default if has unexpected value */
		if (switch_sel > 1000)	{
			printk("\n[sio_switch_init_worker] set SWITCH_SEL(%d) to default 1\n", switch_sel);
			switch_sel = 1;
			sec_set_param_value(__SWITCH_SEL, &switch_sel);
		}
		sec_get_param_value(__SWITCH_SEL, &switch_sel);
