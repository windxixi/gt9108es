...
int get_fuelgauge_ptg_value(void);
...
#if 0
	printk("====== RAW           SOC    : %d ======\n", val);
	printk("====== ADJUSTED(0.3) SOC    : %d ======\n",(val-300)*100/(96100-300));
#endif
...
static int i2c_read(unsigned char reg_addr)
{
	...
	ret = omap_gpio_i2c_read(fuelgauge_i2c_client, &i2c_rd_param);
#else
	...
	if( ret < 0)
	{
		printk("[FG] fail to read max17043\n");
		printk("!!!!!: %d\n", buf[0] << 8 | buf[1]);
		return 0;
	}
#endif
...
static int fuelgauge_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if (!sec_bci->charger.is_charging) {
		...
		batt_ptg = get_fuelgauge_ptg_value();
