...
#elif defined(CONFIG_T1_CHN_CMCC)
#define TOUCH_FIRMWARE_V05  0x05
#define CURRENT_FIRMWARE_VERSION TOUCH_FIRMWARE_V05
#else
...
#elif defined(CONFIG_MACH_SAMSUNG_T1)
	if(touch_version < CURRENT_FIRMWARE_VERSION || touch_version == 0xFF) {
		printk("[touchkey] Force firmware update\n");
		force_firm_update = 1;
		queue_work(touchkey_wq, &touch_update_work);
				  ...
				  const char *buf, size_t size)
{
	...
	if (*buf == 'S' || *buf =='F') {
#if defined(CONFIG_T1_CHN_CMCC)
		if (0)//(*buf !='F' && touch_version == CURRENT_FIRMWARE_VERSION) {
		{
#else
