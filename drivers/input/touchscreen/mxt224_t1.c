...
static void mxt224_ta_probe(int ta_status)
{
	...
	if (!mxt224_enabled) {
		...
		charge_time = copy_data->chrgtime_charging_e; //22
#ifdef CLEAR_MEDIAN_FILTER_ERROR
		...
		active_depth = 35;/* seineTD Only *///34;
		charge_time = copy_data->chrgtime_batt_e; //27;
#ifdef CLEAR_MEDIAN_FILTER_ERROR
...
static irqreturn_t mxt224_irq_thread(int irq, void *ptr)
{
	...
	do {
		...
		{
			//printk(KERN_ERR"[TSP] (MF)Starting irq with 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x", msg[0], msg[1], msg[2], msg[3], msg[4], msg[5], msg[6], msg[7]);
				if((msg[4]&0x5) == 0x5)
				{ 
...
static int __devinit mxt224_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	noise_median.t48_movfilter_for_mferr = 47;
#else
