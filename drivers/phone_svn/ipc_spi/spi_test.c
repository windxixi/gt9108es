...
SPI_TEST_SCENARIO_T spi_test_scenario_mode = SPI_TEST_SCENARIO_NONE;
...
static void _scenario_physical (void)
{
	...
	while(count < 100)
	{
		...
		SPI_OS_TRACE_MID (("spi_scenario_physical test start mrdy %d srdy %d submrdy %d subsrdy %d\n", 
			spi_dev_get_gpio(spi_dev_gpio_mrdy), spi_dev_get_gpio(spi_dev_gpio_srdy),
			spi_dev_get_gpio(spi_dev_gpio_submrdy), spi_dev_get_gpio(spi_dev_gpio_subsrdy)));
		...
		SPI_OS_TRACE_MID(("spi_scenario_physical test waiting for srdy, subsrdy high\n"));
		spi_dev_set_gpio(spi_dev_gpio_mrdy, SPI_DEV_GPIOLEVEL_HIGH);

		do{
		}while (spi_dev_get_gpio(spi_dev_gpio_srdy) == SPI_DEV_GPIOLEVEL_LOW);

		spi_os_sleep(20); // spreadtrum recommend. master should send/receive after that slave is ready.
		...
		else
		{
			...
			for (i = 1; i < SPI_DEV_MAX_PACKET_SIZE; i++)
			{
				...
				{
					failcount++;
					spi_os_trace_dump("spi_scenario_physical receiving fail", &rxbuf[i-8], 16);
					SPI_OS_TRACE_MID (("spi_scenario_physical test receiving fail %d count %d/%d\n", i, failcount, count));
					i = sizeof(rxbuf);
					break;
				}
			}
		...
		do{
		}while (spi_dev_get_gpio(spi_dev_gpio_mrdy) == SPI_DEV_GPIOLEVEL_LOW);

		SPI_OS_TRACE_MID(("spi_scenario_physical test waiting for mrdy, mubsrdy high\n"));
...
#endif
		count++;
		SPI_OS_TRACE_MID (("spi_scenario_physical test receiving result count %d/%d\n", failcount, count));
	}
}
...
static void _scenario_sending (int param)
{
	...
	// param is 0 to fix duration. call this function with param to change timer duration.
	if (param != 0) duration = param;
	...
	do
	{
		...
		if ( app_send_data_to_spi (databystep[step].type, data, databystep[step].size) == 0 )
			// send fail
			spi_os_sleep(50);
		else // send success
			break;
	...
	if (timer_id == 0)
		timer_id = spi_os_create_timer ("spi_test_timer", spi_test_timer_callback, step, duration);
	...
	value = spi_os_start_timer (timer_id, spi_test_timer_callback, step, duration);
...
void spi_test_run (SPI_TEST_SCENARIO_T scenario, int param)
{
	SPI_OS_TRACE_MID (("spi_test_run %d\n", (int) scenario));

	if (scenario == SPI_TEST_SCENARIO_NONE) return;
