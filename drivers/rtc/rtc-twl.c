...
static int twl_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alm)
{
	...
	printk("[BSYSTAR] %s, [%d] %d/%d/%d %d:%d:%d\n", __func__, 
		alm->enabled, alm->time.tm_year, alm->time.tm_mon, alm->time.tm_mday,
...
static int __devinit twl_rtc_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_RTC_CHN_ALARM_BOOT)	// defense_code_bsystar_for_alarmboot 110730
	printk("[BSYSTAR] REG_RTC_INTERRUPTS_REG = %x\n", rtc_irq_bits);
#endif
	if (ret < 0)
		goto out2;
...
#if defined(CONFIG_RTC_CHN_ALARM_BOOT)	// defense_code_bsystar_for_alarmboot 110730
	{
		...
		twl_rtc_read_alarm(&pdev->dev, &alarm_time);
	}
#endif
...
static void twl_rtc_shutdown(struct platform_device *pdev)
{
	...
	if(alarm_en_exit == 1) {
		twl_rtc_set_alarm(&pdev->dev, &autoboot_alm);
	}
	
	sec_get_param_value(__PARAM_INT_14, &temp_param);
	printk("[BSYSTAR] twl_rtc_shutdown __PARAM_INT_14 = %d, %d\n", temp_param, alarm_en_exit);
