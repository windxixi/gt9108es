...
// 1|2010|01|01|00|00
//en yyyy mm dd hh mm
int alarm_en_exit; // defense_code_bsystar_for_alarmboot 110730
...
int alarm_set_alarm(char* alarm_data)
{
	...
	// defense_code_bsystar_for_alarmboot 110730 ++
...
#if defined(CONFIG_RTC_CHN_ALARM_BOOT)
		rtc_tm_to_time(&rtc_current_rtc_time, &rtc_current_time);
		printk("%s, [%d] %d/%d/%d %d:%d:%d\n", __func__, rtc_alarm.enabled, rtc_alarm.time.tm_year-100, rtc_alarm.time.tm_mon+1, 
