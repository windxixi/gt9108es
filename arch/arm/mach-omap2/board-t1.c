...
#include <linux/max14577.h>
#elif defined(CONFIG_FSA9480_MICROUSB)
...
#if defined(CONFIG_MAX14577_MICROUSB)
static void omap4_max14577_reset_cb(void);
#elif defined(CONFIG_FSA9480_MICROUSB)
...
/* For uUSB Switch */
#if defined(CONFIG_MAX14577_MICROUSB)
static struct max14577_platform_data omap4_max14577_pdata = {
	.intb_gpio = OMAP_GPIO_JACK_NINT,
	.usb_cb = omap4_usb_cb,
	.uart_cb = NULL,
	.charger_cb = omap4_charger_cb,
	.jig_cb = omap4_jig_cb,
	.reset_cb = omap4_max14577_reset_cb,
	#ifdef CONFIG_VIDEO_MHL_V1
    	.mhl_cb	    = omap4_mhl_cb,
   	#endif
	#ifdef CONFIG_EXTRA_DOCK_SPEAKER
...
#if defined (CONFIG_MAX14577_MICROUSB)
	printk("\nBoard file [MAX14577]: USB Callback (%d)\n", attached);
#else
...
static void omap4_charger_cb(u8 attached)
{
	/* TODO: need to implement this once uUSB charging available in HW */
#if defined (CONFIG_MAX14577_MICROUSB)
	printk("\nBoard file [MAX14577]: Charger Callback (%d)\n", attached);
#else
...
static void omap4_jig_cb(u8 attached)
{
	/* TODO: need to implement this once uUSB charging available in HW */
#if defined (CONFIG_MAX14577_MICROUSB)
	printk("\nBoard file [MAX14577]: Jig Callback (%d)\n", attached);
#else
	printk("\nBoard file [FSA9480]: Jig Callback \n");
#endif
#if defined ( CONFIG_CHN_CMCC_SPI_SPRD )
        gpio_set_value(OMAP_GPIO_AP_CP_INT1, attached);
#endif
}
...
{
	/* TODO: need to implement this once uUSB charging available in HW */
	printk("\nBoard file [MAX14577]: Reset Callback \n");
}
...
static inline void __init samsung_omap4_battery_init(void)
{
	...
	if (gpio_request(OMAP_GPIO_BAT_REMOVAL, "BAT REMOVAL irq") < 0) {
		...
		samsung_charger_resources[0].start = gpio_to_irq(OMAP_GPIO_BAT_REMOVAL);
...
#define OMAP_GPIO_CHG_EN_REV01 13
	...
	if (gpio_request(gpio_chg_en, "Charge enable gpio") < 0) {
		...
		#if 0 //defined(CONFIG_T1_CHN_CMCC)
		if(system_rev >= 1) {
...
#if 0//( CONFIG_SAMSUNG_REL_HW_REV == 0) //rev 0.0
#define OMAP_GPIO_IPC_MRDY 52
#define OMAP_GPIO_IPC_SRDY 53
#define OMAP_GPIO_IPC_SUB_MRDY 55
#define OMAP_GPIO_IPC_SUB_SRDY 56
#endif
#define OMAP_GPIO_IPC_MRDY 84
#define OMAP_GPIO_IPC_SRDY 86
#define OMAP_GPIO_IPC_SUB_MRDY 90
#define OMAP_GPIO_IPC_SUB_SRDY 91


static struct ipc_spi_platform_data ipc_spi_data = {
	.gpio_ipc_mrdy = OMAP_GPIO_IPC_MRDY,
	.gpio_ipc_srdy = OMAP_GPIO_IPC_SRDY,
	.gpio_ipc_sub_mrdy = OMAP_GPIO_IPC_SUB_MRDY,
	.gpio_ipc_sub_srdy = OMAP_GPIO_IPC_SUB_SRDY,
...
static void ipc_spi_cfg_gpio( void )
{
	...
	// gpio_53 ( IPC_SPI_SRDY ), gpio_52 ( IPC_SPI_MRDY )
	...
	reg_val |= 0xC1030103; // 14,15bit set1 : wakeup event, Muxmod 3, pullup/down disabled ( B -> 3)
	omap_writel( reg_val, gpio_reg_addr );	
	printk( "ipc_spi_cfg_gpio - %s gpio setting done...\n", "IPC_SPI_MRDY" );
	printk( "ipc_spi_cfg_gpio - %s gpio setting done...\n", "IPC_SPI_SRDY" );

	// gpio_55 ( IPC_SPI_SUB_MRDY )
	...
	//reg_val |= 0x010b010b; // Muxmod 3, Muxmod 3
	reg_val |= 0x01030000; // Muxmod 3, Muxmod 3, pullup/down disabled ( B -> 3)
	omap_writel( reg_val, gpio_reg_addr );	
	printk( "ipc_spi_cfg_gpio - %s gpio setting done...\n", "IPC_SPI_SUB_MRDY" );

	// gpio_56 ( IPC_SPI_SUB_SRDY )
	...
	//reg_val |= 0x010b010b; // Muxmod 3, Muxmod 3
	reg_val |= 0x0000C103; // 14,15bit set1 : wakeup event, Muxmod 3, pullup/down disabled ( B -> 3)
	omap_writel( reg_val, gpio_reg_addr );	
	printk( "ipc_spi_cfg_gpio - %s gpio setting done...\n", "IPC_SPI_SUB_SRDY" );
//#elif ( CONFIG_SAMSUNG_REL_HW_REV == 1) //Rev 0.0
#else
	// gpio_84 ( IPC_SPI_MRDY )
	gpio_reg_addr = ( u32 ) ( OMAP4_CTRL_MODULE_PAD_CORE_MUX_PBASE + OMAP4_CTRL_MODULE_PAD_CAM_GLOBALRESET_OFFSET );
	reg_val = omap_readl( gpio_reg_addr );
	reg_val &= 0x0000FFFF; // save origin val
	//reg_val |= 0x010b010b; // Muxmod 3, Muxmod 3
	reg_val |= 0x01030000; // Muxmod 3, Muxmod 3, pullup/down disabled ( B -> 3)
	...
	printk( "ipc_spi_cfg_gpio - %s gpio setting done...\n", "IPC_SPI_MRDY" );
#if 0
	// gpio_86 ( IPC_SPI_SRDY )
	gpio_reg_addr = ( u32 ) ( OMAP4_CTRL_MODULE_PAD_CORE_MUX_PBASE + OMAP4_CTRL_MODULE_PAD_USBB1_ULPITLL_STP_OFFSET );
	reg_val = omap_readl( gpio_reg_addr );
	reg_val &= 0x0000FFFF; // save origin val
	//reg_val |= 0x010b010b; // Muxmod 3, Muxmod 3
	reg_val |= 0xC1030000; // 14,15bit set1 : wakeup event, Muxmod 3, pullup/down disabled ( B -> 3)
	omap_writel( reg_val, gpio_reg_addr );	
	printk( "ipc_spi_cfg_gpio - %s gpio setting done...\n", "IPC_SPI_SRDY" );
#endif	
	// gpio_90 ( IPC_SPI_SUB_MRDY )
	gpio_reg_addr = ( u32 ) ( OMAP4_CTRL_MODULE_PAD_CORE_MUX_PBASE + OMAP4_CTRL_MODULE_PAD_USBB1_ULPITLL_DAT1_OFFSET );
	reg_val = omap_readl( gpio_reg_addr );
	reg_val &= 0x0000FFFF; // save origin val
	//reg_val |= 0x010b010b; // Muxmod 3, Muxmod 3
	reg_val |= 0x01030000; // Muxmod 3, Muxmod 3, pullup/down disabled ( B -> 3)
	...
	printk( "ipc_spi_cfg_gpio - %s gpio setting done...\n", "IPC_SPI_SUB_MRDY" );
#if 0	
	// gpio_91 ( IPC_SPI_SUB_SRDY )
	gpio_reg_addr = ( u32 ) ( OMAP4_CTRL_MODULE_PAD_CORE_MUX_PBASE + OMAP4_CTRL_MODULE_PAD_USBB1_ULPITLL_DAT3_OFFSET );
	reg_val = omap_readl( gpio_reg_addr );
	reg_val &= 0xFFFF0000; // save origin val
	//reg_val |= 0x010b010b; // Muxmod 3, Muxmod 3
	reg_val |= 0x0000C103; // 14,15bit set1 : wakeup event, Muxmod 3, pullup/down disabled ( B -> 3)
	...
	// SPI CLK
	gpio_reg_addr = ( u32 ) ( OMAP4_CTRL_MODULE_PAD_CORE_MUX_PBASE + OMAP4_CTRL_MODULE_PAD_I2C4_SDA_OFFSET );
	reg_val = omap_readl( gpio_reg_addr );
	reg_val &= 0x0000FFFF; // save origin val
	//reg_val |= 0x010b010b; // Muxmod 3, Muxmod 3
	reg_val |= 0x01180000;//0x17080000; //0x01180000; // Muxmod 0, pullup
	omap_writel( reg_val, gpio_reg_addr );

	// SPI SIMO, SOMI
	gpio_reg_addr = ( u32 ) ( OMAP4_CTRL_MODULE_PAD_CORE_MUX_PBASE + OMAP4_CTRL_MODULE_PAD_MCSPI1_SOMI_OFFSET );
	reg_val = omap_readl( gpio_reg_addr );
	reg_val &= 0x00000000; // save origin val
	//reg_val |= 0x010b010b; // Muxmod 3, Muxmod 3
	reg_val |= 0x01000100;//0x37183718; //0x01000100; // Muxmod 0, pulldown
	omap_writel( reg_val, gpio_reg_addr );

	printk("SPI Clk = 0x%08x\n",omap_readl( 0x4A100130));
	printk("SPI SIMO, SOMI = 0x%08x\n",omap_readl( 0x4A100134));
	printk("SRDY(rev0.1) = 0x%08x\n", omap_readl( 0x4A1000D0 ) ); //rev0.1
	printk("SRDY(rev0.0) = 0x%08x\n", omap_readl( 0x4A100078 ) );	 //rev0.0
	...
	if( err ) {
		...
		gpio_direction_output( gpio_ipc_cs_load, 0/*1*/ );	//Heekwon Ko Action1 20110110
	}
#endif
#if 0	
	...
	set_irq_type( OMAP_GPIO_IRQ( OMAP_GPIO_IPC_SUB_SRDY ), IRQ_TYPE_EDGE_RISING );
}
#endif
...
static struct modemctl_platform_data mdmctl_data = {
	...
	.gpio_reset_req_n = OMAP_GPIO_AP_CP_INT2,
...
static struct resource mdmctl_res[] = {
	...
	[1] = {
		.start = OMAP_GPIO_IRQ(OMAP_GPIO_MIPI_HSI_CP_DUMP_INT),	// cp dump int irq
		.end = OMAP_GPIO_IRQ(OMAP_GPIO_MIPI_HSI_CP_DUMP_INT),
		.flags = IORESOURCE_IRQ,
	       },
};
...
static void modemctl_cfg_gpio(void)
{
	...
	printk( "AP FLM TX/RX SET MUX LOW\n" );
...
extern u32 sec_bootmode;
	...
	if (err) {
		...
		gpio_direction_output(gpio_cp_rst, 1); // #if defined( CONFIG_SAMSUNG_MODEL_I9108 )
	//}
#endif	
	...
	set_irq_type(OMAP_GPIO_IRQ(OMAP_GPIO_MIPI_HSI_PHONE_ACTIVE),
		     IRQ_TYPE_EDGE_BOTH);
	set_irq_type(OMAP_GPIO_IRQ(OMAP_GPIO_MIPI_HSI_CP_DUMP_INT),
		     IRQ_TYPE_EDGE_BOTH);
#endif
...
#if defined( CONFIG_MIPI_HSI_LOOPBACK_TEST )
	&mipi_hsi_test,
#elif defined( CONFIG_PHONE_IPC_HSI )
	&ipc_hsi,
#elif defined( CONFIG_PHONE_IPC_SPI_MODULE )
	&ipc_spi,
#endif
...
#if 1 // 20111111 add "if 1"
#define MXT224E_CALCFG_BATT		64
#else // 20111111 add "else"
...
static u8 t9_config_e[] = {TOUCH_MULTITOUCHSCREEN_T9,
				139, 0, 0, 19, 11, 0, 32, MXT224E_THRESHOLD_BATT, 2, 1, 10, 3/*20111111 15*/, 1,
				46/*20111111 11*/, MXT224_MAX_MT_FINGERS, 5, 40, 10, 31, 3,
...
static u8 t25_config_e[] = {SPT_SELFTEST_T25,
				//0, 0, 188, 52, 124, 21, 188, 52, 124, 21, 0, 0, 0, 0}; 20111111
...
static u8 t48_config_e[] = {PROCG_NOISESUPPRESSION_T48,
				3, 132, MXT224E_CALCFG_BATT, 0/*29*/, 0, 0, 0, 10, 15, 0, 0, 0, 0,
				6,	6, 0, 0, 48/*64*/, 4, 48/*64*/, 10, 0, 10/*20*/, 5, 0, 20/*38*/, 0, 5,
				0, 0, 0, 0, 0, 0, 32, MXT224E_THRESHOLD_BATT, 2, 3, 1, 46/*11*/, MXT224_MAX_MT_FINGERS, 5, 40, 10, 10,
...
static u8 t48_config_chrg_e[] = {PROCG_NOISESUPPRESSION_T48,
				...
				6,	6, 0, 0, 64, 4, 64, 10, 0, 9/*20*/, 5, 0, 15/*38*/, 0, 20,
				0, 0, 0, 0, 0, 0, 0, MXT224E_THRESHOLD_CHRG, 2, 5, 2, 47/*46*/, MXT224_MAX_MT_FINGERS, 5, 40, 15, 10,
...
#if defined(CONFIG_MAX14577_MICROUSB)
{
	 ...
	 .platform_data = &omap4_max14577_pdata,
